        ...
        for region_id in self.map.visible_regions:
            if regions[region_id].occupant == self.name:
                ...
                for neighbor in regions[region_id].neighbors:
                    if neighbor.occupant == self.opponent_name:
                        safe_region = False
                        if target is None or neighbor.armies > target.armies:
                            target = neighbor
...
##########################SCOUTING###############################################
                elif target is None and len(to_scout):
    ...
    def process_input(self, cmd):
        if cmd[0] == 'settings':
            ...
            elif cmd[1] == 'wastelands':
                for i in range(2, len(cmd)):
                    self.map.set_wasteland(cmd[i])
            elif cmd[1] == 'opponent_starting_regions':
                for i in range(2, len(cmd)):
                    #keep armies at 5?
                    self.map.update_region(cmd[i], self.opponent_bot, 5)
